...
while (shared_count == NITEMS) // while buffer is full
{
pthread_cond_wait(&shared_buffer_full, &mutex); // this cond_wait is called when

the condition (full buffer) is false
...
while (shared_count == 0) // while buffer is empty
{
pthread_cond_wait(&shared_buffer_empty, &mutex); // this cond_wait is called

when the condition (empty buffer) is false
